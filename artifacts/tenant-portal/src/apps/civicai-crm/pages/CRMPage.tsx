...
import "./crm.css";
import { CalendrierModule }  from "./modules/CalendrierModule";
import { TicketsModule }     from "./modules/TicketsModule";
import { ContratsModule }    from "./modules/ContratsModule";
import { CampagnesModule }   from "./modules/CampagnesModule";
import { EntreprisesModule } from "./modules/EntreprisesModule";
import { AutomationsModule } from "./modules/AutomationsModule";
import { FormulaireModule }  from "./modules/FormulaireModule";
...
const KANBAN_COLS = [
  { key:"Lead froid",     dot:"#5A5F75", contacts: [CONTACTS[4]] },
  { key:"Prospect",       dot:"#38C9F5", contacts: [CONTACTS[3]] },
  { key:"Prospect chaud", dot:"#F5A623", contacts: [CONTACTS[1], CONTACTS[6]] },
  { key:"Négociation",    dot:"#9B6DF5", contacts: [CONTACTS[5]] },
  { key:"Client actif",   dot:"#0FD4A0", contacts: [CONTACTS[0], CONTACTS[6]] },
  { key:"Client fidèle",  dot:"#22C87A", contacts: [CONTACTS[2]] },
];

function ContactsPage({ onSelectContact, onImportCSV }: { onSelectContact:(c:Contact) => void; onImportCSV:() => void }) {
    ...
    <>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16, alignItems:"center", flexWrap:"wrap" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          <button className={`crm-btn ${viewMode==="table"?"crm-btn-primary":"crm-btn-ghost"}`} style={{ fontSize:12 }} onClick={() => setViewMode("table")}>☰ Tableau</button>
          <button className={`crm-btn ${viewMode==="kanban"?"crm-btn-primary":"crm-btn-ghost"}`} style={{ fontSize:12 }} onClick={() => setViewMode("kanban")}>⊞ Kanban</button>
          <button className="crm-btn crm-btn-ghost" style={{ fontSize:12 }}>🔽 Filtres</button>
          <button className="crm-btn crm-btn-ghost" style={{ fontSize:12 }} onClick={onImportCSV}>📤 Import CSV</button>
        </div>
      </div>
          ...
          {KANBAN_COLS.map(col => (
            <div key={col.key} className="crm-kanban-col">
              <div className="crm-kanban-hdr">
                ...
                {col.key}
                <span className="crm-kanban-count">{col.contacts.length}</span>
              </div>
              {col.contacts.map((c, i) => (
                <div key={i} className="crm-kanban-card" onClick={() => onSelectContact(c)}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                    <div className="crm-avatar" style={{ background:c.color, width:28, height:28, fontSize:10 }}>{c.initials}</div>
                    <div>
                      <div className="crm-kanban-card-name">{c.name}</div>
                      <div className="crm-kanban-card-sub">{c.company}</div>
                    </div>
                  </div>
                  <div className="crm-kanban-card-footer">
                    <div className="crm-score-bar" style={{ flex:1 }}>
                      <div className="crm-score-fill" style={{ width:`${c.score}%`, background: c.score>=80?"#0FD4A0":c.score>=55?"#F5A623":"#F55656" }} />
                    </div>
                    <span style={{ fontSize:10, color:"var(--crm-text3)" }}>{c.score}</span>
                    <span style={{ fontSize:10, color:"#0FD4A0", marginLeft:"auto", fontWeight:600 }}>{c.value}</span>
                  </div>
                </div>
              ))}
              <div style={{ margin:"6px 8px 8px", padding:"6px", border:"1px dashed var(--crm-border)", borderRadius:6, textAlign:"center", fontSize:11, color:"var(--crm-text3)", cursor:"pointer" }}>
                + Ajouter
              </div>
            </div>
          ))}
...
const NAV: NavSection[] = [
  ...
  { section:"Commerce", items:[
    ...
    { id:"contrats", icon:"📝", label:"Contrats",             badge:"8" },
    { id:"produits", icon:"📦", label:"Produits & Services" },
  ]},
  { section:"Support", items:[
    { id:"tickets",    icon:"🎫", label:"Tickets support", badge:"5", badgeClass:"amber" },
    { id:"formulaires",icon:"📋", label:"Formulaires web" },
  ]},
  { section:"Analyse", items:[
    { id:"rapports",  icon:"📈", label:"Rapports" },
    { id:"objectifs", icon:"🎯", label:"Objectifs & KPIs" },
    { id:"marketing", icon:"📢", label:"Campagnes" },
  ]},
...
const PAGE_META: Record<CRMPage, { title:string; sub:string; addLabel?:string }> = {
  ...
  marketing:      { title:"Campagnes marketing",   sub:"Séquences, broadcasts et automatisations email",  addLabel:"+ Nouvelle campagne" },
  automatisations:{ title:"Automatisations",       sub:"Flux automatisés actifs",                         addLabel:"+ Nouveau workflow" },
  parametres:     { title:"Paramètres",            sub:"Configuration du compte et de l'équipe" },
  tickets:        { title:"Support client",        sub:"5 tickets ouverts — 3 urgents",                  addLabel:"+ Nouveau ticket" },
  contrats:       { title:"Gestion des contrats",  sub:"8 contrats actifs — 1 en attente de signature",  addLabel:"+ Nouveau contrat" },
  formulaires:    { title:"Formulaires web",       sub:"7 formulaires · 1 199 soumissions ce mois",      addLabel:"+ Créer un formulaire" },
};
...
export function CRMPage() {
  ...
  function renderPage() {
    switch (page) {
      ...
      case "contacts":      return <ContactsPage onSelectContact={setSelectedContact} onImportCSV={() => setShowImportCSV(true)} />;
      case "entreprises":   return <EntreprisesModule />;
      case "pipeline":      return <PipelinePage />;
      case "taches":        return <TachesPage />;
      case "calendrier":    return <CalendrierModule />;
      ...
      case "marketing":     return <CampagnesModule />;
      case "automatisations": return <AutomationsModule />;
      case "formulaires":   return <FormulaireModule />;
  ...
  return (
    <div className="crm-root">
      ...
      <nav className="crm-sidebar">
        ...
        {/* Back to portal */}
        <button className="crm-back-btn" onClick={() => setLocation("/dashboard")}>
          ← Retour au portail
        </button>
          ...
          <div key={section.section}>
            ...
            {section.items.map(item => (
              ...
              >
                ...
                {item.label}
                {item.badge && (
                  <span className={`crm-nav-badge ${item.badgeClass ?? ""}`}>{item.badge}</span>
                )}
              </div>
            ))}
          </div>
      ...
      <main className="crm-main">
        ...
        <header className="crm-header">
          ...
          <div className="crm-search-shortcut" onClick={() => setShowSearch(true)}>
            <span style={{ fontSize:14, color:"var(--crm-text3)" }}>🔍</span>
            <span style={{ fontSize:13, color:"var(--crm-text3)" }}>Rechercher…</span>
            <kbd style={{ background:"var(--crm-bg4)", border:"1px solid var(--crm-border)", borderRadius:4, padding:"1px 6px", fontSize:11, color:"var(--crm-text3)", fontFamily:"monospace" }}>⌘K</kbd>
          </div>
          <div className="crm-header-actions">
            ...
            {meta.addLabel && (
              <button className="crm-btn crm-btn-primary" onClick={() => {
                if (page === "contacts") setShowAddModal(true);
              }}>
                {meta.addLabel}
              </button>
            )}
