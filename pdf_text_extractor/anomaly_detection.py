...
def _geolocate(ip: str) -> dict[str, str]:
    ...
    try:
        r = httpx.get(f"https://ipinfo.io/{ip}/json", headers={"Accept": "application/json"}, timeout=3)
        ...
        with get_db() as cur:
            cur.execute(
                """INSERT INTO security_alerts
                   ...
                   VALUES (%s, %s, %s, %s, %s, %s)""",
                (org_id, user_id, alert_type, severity, json.dumps(details, default=str), ip_address),
            )
    except Exception:
        pass
