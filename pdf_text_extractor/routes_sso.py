...
async def sso_callback(request: Request, code: str = "", state: str = "", error: str = ""):
    ...
    email = (
        (
            ui.get("mail")  # Microsoft Graph
            or ui.get("userPrincipalName")
            or ui.get("email")
    ...
    full_name = (
        ui.get("displayName")
        or ui.get("name")
        or f"{ui.get('givenName','')} {ui.get('familyName','')}".strip()
        or email
    )
    ...
    return RedirectResponse(
        url=f"{APP_URL}?sso_email={encoded_email}&org_slug={urllib.parse.quote(org_id)}", status_code=302
    )
