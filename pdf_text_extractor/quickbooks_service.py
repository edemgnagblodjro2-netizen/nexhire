...
def query_quickbooks(category: str, org_id: str,
    ...
    try:
        ...
        if category == "expenses":
            rows = _query(realm_id, creds,
                "SELECT Id, TxnDate, TotalAmt, AccountRef, EntityRef "
                "FROM Bill ORDERBY TxnDate DESC MAXRESULTS 20")
            total = sum(float(r.get("TotalAmt", 0)) for r in rows)
            return {
