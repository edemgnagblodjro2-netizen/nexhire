...
def query_epicor(category: str, org_id: str, period: str = "current_month") -> dict:
    ...
    if not api_url or not username or not password or not company_id:
        return {
            "error": "Credentials Epicor incomplets — reconfigurer le connecteur (URL, utilisateur, mot de passe, Company ID)"
        }
    ...
    try:
        if category == "production_orders":
            r = httpx.get(
                f"{api_url}/Erp.BO.JobEntrySvc/JobHeads",
                headers=hdrs,
                params={
                    **params,
            ...
            en_retard = [j for j in en_cours if j.get("DueDate", "9999") < today_str]
            return {
                ...
                "retards": [
                    {
                        "ordre": j.get("JobNum"),
                        "article": j.get("PartNum"),
                        "description": j.get("JobDescription"),
                        "échéance": j.get("DueDate"),
                    }
        ...
        if category == "inventory":
            r = httpx.get(
                f"{api_url}/Erp.BO.PartSvc/Parts",
                headers=hdrs,
                params={
                    **params,
            ...
            sous_seuil = [p for p in parts if 0 < p.get("OnHandQty", 0) < p.get("MinStockQty", 0)]
            valeur = sum(p.get("OnHandQty", 0) * p.get("UnitPrice", 0) for p in parts)
            return {
                ...
                "alertes": [
                    {
                        "article": p.get("PartNum"),
                        "nom": p.get("PartDescription"),
                        "stock": p.get("OnHandQty"),
                        "min_requis": p.get("MinStockQty"),
                    }
        ...
        if category == "purchasing":
            r = httpx.get(
                f"{api_url}/Erp.BO.POSvc/POHeaders",
                headers=hdrs,
                params={
                    **params,
            ...
            return {
                ...
                "commandes": [
                    {
                        "no": p.get("PONum"),
                        "fournisseur": p.get("VendorName"),
                        "montant": p.get("TotalOrderAmt"),
                        "échéance": p.get("DueDate"),
                    }
                    for p in pos[:10]
                ],
                "source": "epicor",
        ...
        if category == "financials":
            r = httpx.get(
                f"{api_url}/Erp.BO.GLJrnDtlSvc/GLJrnDtls",
                headers=hdrs,
                params={
                    **params,
