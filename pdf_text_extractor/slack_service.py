...
def search_slack(query: str, org_id: str, channel: str | None = None, limit: int = 5) -> list[dict]:
    ...
    try:
        ...
        r = httpx.get(f"{_BASE}/search.messages", headers=_user_headers(creds), params=params, timeout=10)
        r.raise_for_status()
        data = r.json()
        if not data.get("ok"):
    ...
    if len(results) < limit:
        try:
            r = httpx.get(
                f"{_BASE}/files.list",
                headers=_bot_headers(creds),
                params={"count": min(5, limit - len(results))},
                timeout=10,
            )
            r.raise_for_status()
            data = r.json()
...
def list_channels(org_id: str, limit: int = 200) -> list[dict]:
    ...
    try:
        while True:
            ...
            r = httpx.get(f"{_BASE}/conversations.list", headers=_bot_headers(creds), params=params, timeout=12)
            r.raise_for_status()
            data = r.json()
            if not data.get("ok"):
                break
