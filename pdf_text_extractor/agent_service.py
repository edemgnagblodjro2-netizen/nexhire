        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "status": {
                        "type": "string",
                        "enum": ["open", "in_progress", "done", "overdue", "all"],
                        "default": "all",
                    },
                    "limit": {"type": "integer", "default": 5},
                },
...
def _internal_contracts(org_id: str, filter: str = "all", category: str | None = None, limit: int = 10) -> dict:
    try:
        ...
        q = (
            sb.table("contracts")
            .select(
                "vendor, description, category, annual_value, currency, renewal_date, negotiation_potential, status, departments(name)"
            )
            .eq("organization_id", org_id)
        )
        if category:
...
def _internal_processes(org_id: str, filter: str = "all", limit: int = 10) -> dict:
    try:
        ...
        rows = (
            service_client()
            .table("workforce_processes")
            .select(
                "name, description, team_size, manual_hours_per_month, automation_potential, hourly_cost, status, departments(name)"
            )
            .eq("organization_id", org_id)
            .execute()
            .data
...
def _mock_servicenow(query: str, status: str = "all", priority: str = "all", limit: int = 5) -> list[dict]:
    data = [
        {
            "id": "INC0012847",
            "titre": "Panne serveur de production DB-01",
            "priorité": "Critique",
            "statut": "Ouvert",
            "assigné": "Jean Tremblay",
            "créé": "2026-06-04T08:15:00Z",
        },
        {
            "id": "INC0012851",
            "titre": "VPN lent — utilisateurs distants Montréal",
            "priorité": "Haute",
            "statut": "En cours",
            "assigné": "Marie Côté",
            "créé": "2026-06-04T10:30:00Z",
        },
        {
            "id": "INC0012839",
            "titre": "Certificat SSL expiré — portail RH",
            "priorité": "Critique",
            "statut": "Ouvert",
            "assigné": "Non assigné",
            "créé": "2026-06-03T14:00:00Z",
        },
        {
            "id": "INC0012820",
            "titre": "Imprimante hors ligne — Salle 204",
            "priorité": "Basse",
            "statut": "Ouvert",
            "assigné": "Support L1",
            "créé": "2026-06-02T09:00:00Z",
        },
    ]
    if priority != "all":
...
def _mock_jira(query: str, status: str = "all", project: str | None = None, limit: int = 5) -> list[dict]:
    data = [
        {
            "id": "NEXH-847",
            "titre": "Intégration API Salesforce — phase 2",
            "statut": "En retard",
            "sprint": "Sprint 23",
            "assigné": "Dev Team",
            "échéance": "2026-05-30",
        ...
        {
            "id": "NEXH-831",
            "titre": "Refactoring module paiement",
            "statut": "Bloqué",
            "sprint": "Sprint 23",
            "assigné": "Fatima Diallo",
            "échéance": "2026-06-01",
            "retard_jours": 4,
        },
        {
            "id": "NEXH-862",
            "titre": "Tests de charge — endpoint /api/agent",
            "statut": "À faire",
            "sprint": "Sprint 24",
            "assigné": "QA Team",
            "échéance": "2026-06-10",
        ...
        {
            "id": "NEXH-855",
            "titre": "Dashboard analytique — composant graphe",
            "statut": "En cours",
            "sprint": "Sprint 24",
    ...
    data = [
        {
            "type": "email",
            "de": "pdg@client.ca",
            "objet": "Re: Renouvellement contrat Q3",
            "date": "2026-06-05T07:42:00Z",
            "lu": False,
        },
        {
            "type": "email",
            "de": "finance@fournisseur.com",
            "objet": "Facture #INV-2026-0847 — échéance 15 juin",
            "date": "2026-06-04T16:10:00Z",
            "lu": False,
        },
        {
            "type": "sharepoint",
            "titre": "Politique achats v4.2.pdf",
            "modifié_par": "DG Admin",
            "date": "2026-06-01T10:00:00Z",
        },
        {
            "type": "teams",
            "canal": "Direction Générale",
            "message": "Rappel : réunion budget vendredi 10h",
...
def _mock_zendesk(query: str, status: str = "all", priority: str = "all", limit: int = 5) -> list[dict]:
    data = [
        ...
        {
            "id": "ZD-4798",
            "sujet": "Demande d'exportation des données RGPD",
            "priorité": "normal",
            "statut": "open",
            "client": "Client Interne",
            "créé": "2026-06-02T10:45:00Z",
            "assigné": "Équipe Conformité",
        },
        {
            "id": "ZD-4790",
    ...
    data = [
        {
            "id": "AT-12047",
            "type": "ticket",
            "titre": "Mise à jour firmware — switches Cisco Laval",
            "statut": "in_progress",
            "sla": "4h",
            "temps_restant": "1h30",
        ...
        },
        {
            "id": "AT-12039",
            "type": "ticket",
            "titre": "Déploiement antivirus — 45 postes",
            "statut": "new",
            "sla": "8h",
            "priorité": "high",
        ...
        },
        {
            "id": "AT-P-087",
            "type": "project",
            "titre": "Migration Exchange → Microsoft 365",
            "statut": "in_progress",
            "avancement": "67 %",
            "échéance": "2026-07-15",
        ...
        },
        {
            "id": "AT-C-023",
            "type": "contract",
            "titre": "Contrat support annuel — niveau Gold",
            "statut": "active",
        ...
        },
        {
            "id": "AT-12031",
            "type": "ticket",
            "titre": "VPN Cisco AnyConnect — erreur certificat",
            "statut": "waiting",
            "sla": "2h",
            "en_attente": "Approbation client",
            "technicien": "Marie Côté",
        },
    ]
    if type != "all":
    ...
    return [
        {
            "id": "msg_001",
            "type": "email",
            "expéditeur": "pdg@client.ca",
            "sujet": "Re: Réunion Q3 — budget annuel",
            "date": "2026-06-06",
            "statut": "Non lu",
        },
        {
        ...
        },
        {
            "id": "meet_003",
            "type": "calendar",
            "titre": "Comité de direction mensuel",
            "date": "2026-06-10",
            "participants": 8,
        },
        {
            "id": "msg_004",
            "type": "email",
            "expéditeur": "rh@organisation.ca",
            "sujet": "Politique congés été 2026 — mise à jour",
            "date": "2026-06-04",
            "statut": "Lu",
        },
    ][:limit]
    ...
    return [
        {
            "canal": "#direction",
            "auteur": "Marie T.",
            "message": "Le rapport d'audit Q2 est prêt pour révision.",
            "date": "2026-06-06T14:30:00Z",
        },
        {
            "canal": "#it-support",
            "auteur": "Jean P.",
            "message": "⚠️ Certificat SSL du portail RH expire demain — action requise.",
            "date": "2026-06-06T10:15:00Z",
        },
        {
            "canal": "#finances",
            "auteur": "Sophie L.",
            "message": "Dépassement budgétaire Azure détecté : +15% au-dessus du plafond mensuel.",
            "date": "2026-06-05T16:45:00Z",
        },
        {
            "canal": "#general",
            "auteur": "Paul D.",
            "message": "Rappel : réunion budget annuel vendredi 09h00 salle A.",
...
def _mock_adp(category: str, period: str = "current_month") -> dict:
    mocks = {
        "payroll": {
            "masse_salariale_brute": "1 245 000 CAD",
            "nb_employés_payés": 138,
            "charges_patronales": "186 750 CAD",
            "période": period,
        },
...
def _mock_asana(query: str, status: str = "all", limit: int = 5) -> list[dict]:
    return [
        {
            "id": "T-1001",
            "titre": "Lancement nouveau portail client",
            "projet": "Digital Transformation Q3",
            "statut": "En cours",
            "assigné": "Marie C.",
            "échéance": "2026-06-30",
            "priorité": "Haute",
        ...
        {
            "id": "T-1002",
            "titre": "Revue des contrats fournisseurs TI",
            "projet": "Optimisation Achats",
            "statut": "En retard",
            "assigné": "Jean T.",
            "échéance": "2026-06-01",
            "priorité": "Critique",
        ...
        {
            "id": "T-1003",
            "titre": "Formation équipe sur nouveaux outils",
            "projet": "Onboarding Tools",
            "statut": "À faire",
            "assigné": "RH",
            "échéance": "2026-06-20",
            "priorité": "Normale",
        ...
        {
            "id": "T-1004",
            "titre": "Audit de conformité SOC 2",
            "projet": "Sécurité & Conformité",
            "statut": "En cours",
            "assigné": "Paul D.",
            "échéance": "2026-07-15",
            "priorité": "Haute",
...
def _mock_monday(query: str, board: str | None = None, status: str = "all", limit: int = 5) -> list[dict]:
    return [
        {
            "tableau": "Projets 2026",
            "item": "Migration infrastructure Azure",
            "statut": "En cours",
            "propriétaire": "IT",
            "date_fin": "2026-07-15",
        },
        {
            "tableau": "Suivi Clients",
            "item": "Renouvellement contrat ABC Corp",
            "statut": "À risque",
            "propriétaire": "Ventes",
            "date_fin": "2026-06-30",
        },
        {
            "tableau": "Marketing Q3",
            "item": "Campagne LinkedIn B2B",
            "statut": "Planifié",
        ...
        {
            "tableau": "Projets 2026",
            "item": "Déploiement NexHire — Phase 2",
            "statut": "En retard",
            "propriétaire": "IT",
            "date_fin": "2026-06-10",
        },
...
def _mock_clickup(query: str, status: str = "all", limit: int = 5) -> list[dict]:
    return [
        ...
        {
            "id": "CU-003",
            "titre": "Déploiement patch sécurité serveurs",
            "espace": "IT Operations",
            "statut": "Bloqué",
            "assigné": "Marc L.",
            "priorité": "Critique",
...
def _mock_quickbooks(category: str, period: str = "current_month") -> dict:
    mocks = {
        "invoices": {
            "factures_envoyées": 47,
            "montant_total": "234 500 CAD",
            "en_attente_paiement": 12,
            "en_retard": 3,
            "valeur_retard": "28 400 CAD",
        },
        "expenses": {
            "dépenses_total": "89 300 CAD",
            "catégories_top": ["Logiciels: 32 000$", "Déplacements: 18 200$", "Bureau: 12 500$"],
        },
        "accounts": {"actif_courant": "1 450 000 CAD", "passif_courant": "380 000 CAD", "trésorerie": "290 000 CAD"},
        "profit_loss": {
            "revenus": "485 000 CAD",
            "dépenses": "312 000 CAD",
            "bénéfice_net": "173 000 CAD",
            "marge": "35.7%",
        },
...
def _mock_aws(category: str, period: str = "current_month") -> dict:
    mocks = {
        "costs": {
            "coût_total": "28 450 USD",
            "variation_mois": "+12%",
            "top_services": ["EC2: 12 300$", "S3: 4 200$", "RDS: 3 800$", "Lambda: 1 200$"],
            "alerte": "Dépassement budget de 15%",
        },
        "instances": {
            "total_ec2": 34,
            "actives": 28,
            "arrêtées": 6,
            "optimisation": "4 instances sous-utilisées — économie estimée 1 800 USD/mois",
        },
...
def _mock_intune(category: str, status: str = "all") -> dict:
    mocks = {
        ...
        "compliance": {
            ...
            "non_conformes": 9,
            "pourcentage": "95.2%",
            "problèmes_top": ["BitLocker désactivé: 5", "Antivirus obsolète: 3", "OS non à jour: 1"],
        },
...
def _mock_crowdstrike(query: str, severity: str = "all", limit: int = 5) -> list[dict]:
    return [
        {
            "id": "DET-2847",
            "titre": "Tentative de mouvement latéral détectée",
            "sévérité": "Critique",
            "hôte": "WRK-0142",
            "statut": "En cours",
            "détecté": "2026-06-06T03:12:00Z",
        },
        {
            "id": "DET-2841",
            "titre": "Exécution de script PowerShell suspect",
            "sévérité": "Haute",
            "hôte": "SRV-DB01",
            "statut": "Résolu",
            "détecté": "2026-06-05T22:45:00Z",
        },
        {
            "id": "DET-2835",
            "titre": "Connexion depuis IP inhabituelle (géolocalisation étrangère)",
            "sévérité": "Moyenne",
            "hôte": "USR-sarah.m",
            "statut": "Résolu",
            "détecté": "2026-06-05T14:30:00Z",
        },
    ][:limit]
...
def _mock_epicor(category: str = "production_orders", period: str = "current_month") -> dict:
    mocks = {
        "production_orders": {
            ...
            "en_cours": 7,
            "top_retards": [
                {"ordre": "MFG-2847", "article": "Composant A-412", "retard_jours": 8},
                {"ordre": "MFG-2831", "article": "Assemblage B-201", "retard_jours": 3},
            ],
        },
        "inventory": {
            "articles_total": 1240,
    ...
    data = [
        {
            ...
            "étape": "Proposition envoyée",
            "probabilité": "70 %",
            "contact": "Directeur TI",
            "fermeture_prévue": "2026-07-31",
        },
        {
            "type": "deal",
        ...
        },
        {
            ...
            "probabilité": "45 %",
            "contact": "VP Innovation",
            "fermeture_prévue": "2026-08-15",
        },
    ]
    if object_type != "all":
        data = [d for d in data if d["type"] == object_type]
...
def _call_tool(name: str, arguments: dict[str, Any], org_id: str | None = None) -> tuple[Any, bool]:
    ...
    if name in _INTERNAL_TOOL_NAMES and org_id:
        ...
        if name == "query_contracts":
            return (
                _internal_contracts(
                    org_id, arguments.get("filter", "all"), arguments.get("category"), arguments.get("limit", 10)
                ),
                False,
            )
    ...
    if org_id:
        ...
        try:
            if name == "search_microsoft_365":
                ...
                _real_result = _fn(
                    category=arguments.get("category", "budget"),
                ...
                _real_result = _fn(
                    category=arguments.get("category", "headcount"),
                    department=arguments.get("department"),
                ...
                _real_result = _fn(
                    category=arguments.get("category", "invoices"),
                    org_id=org_id,
                    period=arguments.get("period", "current_month"),
                )
                _real_matched = True
                ...
                _real_result = _fn(
                    category=arguments.get("category", "costs"),
                    org_id=org_id,
                    period=arguments.get("period", "current_month"),
                )
                _real_matched = True
                ...
                _real_result = _fn(
                    category=arguments.get("category", "financials"),
                    org_id=org_id,
                    period=arguments.get("period", "current_month"),
                )
                _real_matched = True
                ...
                _real_result = _fn(
                    category=arguments.get("category", "devices"), org_id=org_id, department=arguments.get("department")
                )
                _real_matched = True
                ...
                _real_result = _fn(
                    category=arguments.get("category", "production_orders"),
                    org_id=org_id,
                    period=arguments.get("period", "current_month"),
                )
                _real_matched = True
            ...
            try:
                ...
                if org_id:
                    try:
                        ...
                        _store_alert(org_id, user_id, "prompt_injection", "high", {"question": question[:300]})
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    if choice.finish_reason == "tool_calls" and choice.message.tool_calls:
        for tool_call in choice.message.tool_calls:
            ...
            if fn_name in _INTERNAL_TOOL_NAMES:
                ...
                source = next(
                    (c for c, t in connector_tool_map.items() if t == fn_name and c not in _INTERNAL_TOOL_NAMES),
                    fn_name,
                )
