...
def query_intune(category: str, org_id: str,
    ...
    try:
        ...
        if category == "devices":
            r = httpx.get(f"{_GRAPH}/deviceManagement/managedDevices",
                          headers=headers,
                          params={"$select": "id,deviceName,operatingSystem,complianceState,lastSyncDateTime,userPrincipalName",
                                  "$top": 20},
                          timeout=12)
            r.raise_for_status()
            ...
            return {
                ...
                "appareils": [
                    {"nom": d.get("deviceName"), "os": d.get("operatingSystem"),
        ...
        if category == "compliance":
            r = httpx.get(f"{_GRAPH}/deviceManagement/deviceCompliancePolicies",
            ...
            return {
                ...
                "politiques": [
                    {"nom": p.get("displayName"), "plateforme": p.get("@odata.type", "").replace("#microsoft.graph.", ""),
        ...
        if category == "apps":
            r = httpx.get(f"{_GRAPH}/deviceAppManagement/mobileApps",
                          headers=headers,
                          params={"$select": "id,displayName,publisher,publishingState",
                                  "$top": 20},
                          timeout=12)
            r.raise_for_status()
            ...
            return {
                ...
                "liste": [
                    {"nom": a.get("displayName"), "éditeur": a.get("publisher"),
                     "statut": a.get("publishingState")}
                    for a in apps
                ],
            }
