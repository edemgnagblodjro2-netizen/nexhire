...
def _get_token(base_url: str, client_id: str, client_secret: str) -> str:
    r = httpx.post(f"{base_url}/oauth2/token",
...
def search_crowdstrike(query: str, org_id: str,
                       severity: str = "all", limit: int = 5) -> list[dict]:
    ...
    try:
        ...
        r = httpx.get(f"{base_url}/detects/queries/detects/v1",
                      headers=headers,
        ...
        det_r = httpx.post(f"{base_url}/detects/entities/summaries/GET/v1",
                           headers=headers,
                           json={"ids": ids[:limit]},
                           timeout=12)
