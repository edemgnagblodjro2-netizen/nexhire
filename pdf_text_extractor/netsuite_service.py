...
def _oauth1_header(
    method: str, url: str, account_id: str, consumer_key: str, consumer_secret: str, token_id: str, token_secret: str
) -> str:
    ...
    base_str = "&".join(
        [
            ...
            urllib.parse.quote(
                "&".join(f"{urllib.parse.quote(k)}={urllib.parse.quote(v)}" for k, v in sorted(params.items())), safe=""
            ),
        ]
    )
...
def query_netsuite(category: str, org_id: str, period: str = "current_month") -> dict:
    ...
    try:
        ...
        auth_header = _oauth1_header("GET", url, account_id, consumer_key, consumer_secret, token_id, token_secret)
        r = httpx.get(
            url,
            headers={"Authorization": auth_header, "Accept": "application/json", "Prefer": "transient"},
            params={"limit": 10},
            timeout=15,
        )
        r.raise_for_status()
