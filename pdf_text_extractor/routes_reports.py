...
def _chart_image(spec: ChartSpec, w: float = 6.5, h: float = 3.6) -> bytes:
    ...
    if spec.type == "pie":
        ...
        x = range(n)
        if spec.type == "bar":
            bars = ax.bar(x, spec.values, color=palette[:n], width=0.55, edgecolor="white", linewidth=1.2)
            for bar, val in zip(bars, spec.values):
                ax.text(
                    bar.get_x() + bar.get_width() / 2,
                    bar.get_height() + max(spec.values) * 0.01,
                    f"{val:g}",
                    ha="center",
                    va="bottom",
                    fontsize=8.5,
...
def _export_pdf(
    ...
    s_title = ParagraphStyle(
        "title", fontName="Helvetica-Bold", fontSize=18, textColor=C_NAVY, spaceAfter=4, spaceBefore=6
    )
    s_date = ParagraphStyle("date", fontName="Helvetica", fontSize=10, textColor=C_GRAY, spaceAfter=10)
    s_label = ParagraphStyle(
        "label", fontName="Helvetica-Bold", fontSize=9, textColor=C_BRAND, spaceBefore=14, spaceAfter=4
    )
    ...
    period_line = (
        f"Période : {payload.period_label}   ·   Généré le {date_str}"
        if payload.period_label
        else f"Généré le {date_str}"
    )
    ...
    if budget_dept:
        ...
        try:
            spec_dept = ChartSpec(
                type="bar", title="% Budget consommé par département", labels=bar_labels, values=bar_pct
            )
    ...
    if budget_cat:
        ...
        for r in budget_cat:
            ...
            cat_vals.append(round(actual, 0))
        ...
        try:
            spec_cat = ChartSpec(
                type="pie", title="Répartition des dépenses par catégorie", labels=cat_labels, values=cat_vals
            )
    ...
    return StreamingResponse(
        buf, media_type="application/pdf", headers={"Content-Disposition": f'attachment; filename="{_filename("pdf")}"'}
    )
...
def _export_excel(
    ...
    if budget_dept:
        ...
        for col in "BCDE":
            bws.column_dimensions[col].width = 18
        ...
        for col, label in zip(
            "ABCDE", ["Département", f"Alloué ({currency})", f"Réel ({currency})", f"Écart ({currency})", "% Consommé"]
        ):
    ...
    if budget_cat:
        ...
        for col in "BCD":
            cws.column_dimensions[col].width = 18
...
def _export_word(
    ...
    rd = pd.add_run(period_word)
    rd.font.size = Pt(10)
    rd.font.color.rgb = GRAY_W
    ...
    if payload.charts:
        ...
        for spec in payload.charts:
            try:
                ...
                cap.runs[0].font.color.rgb = GRAY_W
            except Exception:
                pass
        doc.add_paragraph()
    ...
    if budget_dept:
        ...
        for r in budget_dept:
            ...
            for i, v in enumerate(
                [r.get("name") or "—", f"{alloc:,.0f}", f"{actual:,.0f}", f"{alloc-actual:,.0f}", f"{pct:.1f}%"]
            ):
                row_cells[i].text = v
        doc.add_paragraph()
    ...
    if budget_cat:
        ...
        for r in budget_cat:
            ...
            for i, v in enumerate(
                [r.get("name") or "—", f"{r.get('allocated') or 0:,.0f}", f"{actual:,.0f}", f"{pct:.1f}%"]
            ):
                rc[i].text = v
        doc.add_paragraph()
...
def _word_org_title(doc, org_name: str, navy, brand):
    ...
    p = doc.add_paragraph()
    r1 = p.add_run(org_name)
    r1.font.size = Pt(22)
    r1.font.bold = True
    r1.font.color.rgb = navy
    r2 = p.add_run("  ·  NexHire EIP")
    r2.font.size = Pt(14)
    r2.font.bold = False
...
def _wlabel(doc, text: str, color):
    ...
    r.font.size = 9
    r.font.bold = True
    r.font.color.rgb = color  # type: ignore
...
def _export_pptx(
    ...
    def add_header_bar(slide):
        ...
        bar.fill.solid()
        bar.fill.fore_color.rgb = NAVY_P
        bar.line.fill.background()
        # accent stripe
        acc = slide.shapes.add_shape(1, 0, 0, Inches(0.13), Inches(0.82))
        acc.fill.solid()
        acc.fill.fore_color.rgb = BRAND_P
    ...
    bx2 = tb(s1, 1, 3.0, 11, 1.5)
    p2 = bx2.text_frame.paragraphs[0]
    p2.alignment = PP_ALIGN.CENTER
    run(p2, title, 24, bold=True, color=WHITE_P)
    bx3 = tb(s1, 1, 4.8, 11, 0.5)
    p3 = bx3.text_frame.paragraphs[0]
    p3.alignment = PP_ALIGN.CENTER
    cover_date = f"{payload.period_label}  ·  {date_str}" if payload.period_label else date_str
    run(p3, cover_date, 13, color=GRAY_P)
    if payload.sources:
        bx4 = tb(s1, 1, 5.5, 11, 0.5)
        p4 = bx4.text_frame.paragraphs[0]
        p4.alignment = PP_ALIGN.CENTER
        run(p4, " · ".join(s.upper() for s in payload.sources), 12, color=BRAND_P)

    # ── Slide 2 : Content
    s2 = prs.slides.add_slide(blank)
    bg2 = s2.background.fill
    bg2.solid()
    bg2.fore_color.rgb = LIGHT_P
    add_header_bar(s2)
    bt = tb(s2, 0.3, 0.95, 12.7, 0.65)
    p_t = bt.text_frame.paragraphs[0]
    run(p_t, title, 18, bold=True, color=NAVY_P)

    y = 1.75
    if payload.sources:
        bsr = tb(s2, 0.3, y, 12.7, 0.4)
        psr = bsr.text_frame.paragraphs[0]
        run(psr, "Sources : " + " · ".join(s.upper() for s in payload.sources), 10, color=BRAND_P)
        y += 0.5

    bq = tb(s2, 0.3, y, 12.7, 0.5)
    pq = bq.text_frame.paragraphs[0]
    run(pq, f"Q : {payload.question[:200]}", 10, color=GRAY_P)
    y += 0.6

    ba = tb(s2, 0.3, y, 12.7, 7.5 - y - 0.2)
    tf = ba.text_frame
    for i, line in enumerate([l for l in payload.answer.split("\n") if l.strip()][:20]):
        ...
        r.text = line
        r.font.size = Pt(10.5)
        r.font.color.rgb = NAVY_P

    # ── Slides 3+ : Charts
    for spec in payload.charts:
        try:
            sg = prs.slides.add_slide(blank)
            bgg = sg.background.fill
            bgg.solid()
            bgg.fore_color.rgb = LIGHT_P
            add_header_bar(sg)
            bct = tb(sg, 0.3, 0.95, 12.7, 0.65)
            p_ct = bct.text_frame.paragraphs[0]
            ...
            ct = (
                XL_CHART_TYPE.PIE
                if spec.type == "pie"
                else XL_CHART_TYPE.LINE if spec.type == "line" else XL_CHART_TYPE.COLUMN_CLUSTERED
            )
    ...
    if budget_dept:
        try:
            ...
            bgd = sd.background.fill
            bgd.solid()
            bgd.fore_color.rgb = LIGHT_P
            add_header_bar(sd)
            btd = tb(sd, 0.3, 0.95, 12.7, 0.65)
            p_td = btd.text_frame.paragraphs[0]
            ...
            ch_b = sd.shapes.add_chart(
                XL_CHART_TYPE.COLUMN_CLUSTERED, Inches(0.5), Inches(1.7), Inches(12.3), Inches(5.5), cd_b
            )
    ...
    if budget_cat:
        try:
            ...
            bgc2 = sc2.background.fill
            bgc2.solid()
            bgc2.fore_color.rgb = LIGHT_P
            add_header_bar(sc2)
            btc2 = tb(sc2, 0.3, 0.95, 12.7, 0.65)
            p_tc2 = btc2.text_frame.paragraphs[0]
    ...
    bgc = sc.background.fill
    bgc.solid()
    bgc.fore_color.rgb = NAVY_P
    add_header_bar(sc)
    bct2 = tb(sc, 0.5, 1.0, 12.3, 0.7)
    pct2 = bct2.text_frame.paragraphs[0]
    run(pct2, "Conclusion", 22, bold=True, color=WHITE_P)
    bcb = tb(sc, 0.5, 2.0, 12.3, 4.5)
    tf_c = bcb.text_frame
    for i, line in enumerate([l for l in conclusion.split("\n") if l.strip()]):
        ...
        r.text = line
        r.font.size = Pt(12)
        r.font.color.rgb = GRAY_P
    bdate = tb(sc, 0.5, 6.8, 12.3, 0.4)
    pd = bdate.text_frame.paragraphs[0]
