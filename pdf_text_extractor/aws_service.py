...
def _boto_query(service: str, region: str, access_key: str, secret_key: str, action: str, payload: dict) -> dict:
...
def query_aws(category: str, org_id: str, period: str = "current_month") -> dict:
    ...
    try:
        if category == "costs":
            ...
            top = sorted(groups, key=lambda g: float(g["Metrics"]["BlendedCost"]["Amount"]), reverse=True)[:5]
            return {
                "coût_total": f"{float(total_str):,.2f} USD",
                "période": f"{first_day} → {last_day}",
                "top_services": [
                    {"service": g["Keys"][0], "coût": f"{float(g['Metrics']['BlendedCost']['Amount']):,.2f} USD"}
                    for g in top
                ],
            }

        if category == "ec2":
            resp = _boto_query("ec2", region, access_key, secret_key, "describe_instances", {"MaxResults": 20})
                ...
                "instances": [
                    {
                        ...
                        "nom": next((t["Value"] for t in i.get("Tags", []) if t["Key"] == "Name"), "—"),
                    }
        ...
        if category == "s3":
            resp = _boto_query("s3", region, access_key, secret_key, "list_buckets", {})
            buckets = resp.get("Buckets", [])
            return {
                "buckets_total": len(buckets),
                "liste": [{"nom": b.get("Name"), "créé": str(b.get("CreationDate", ""))} for b in buckets[:20]],
            }
