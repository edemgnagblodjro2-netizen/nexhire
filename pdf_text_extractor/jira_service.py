...
def _get_cloud_id(creds: dict) -> str | None:
    try:
...
def search_jira(
    ...
    cloud_id = creds.get("cloud_id") or _get_cloud_id(creds)
    if not cloud_id:
        return [{"error": "Impossible d'accéder au cloud Jira — vérifiez les permissions OAuth"}]
