...
def get_workspace_info(org_id: str) -> dict:
    ...
    try:
        ...
        try:
            rd = httpx.get(
                f"{_DRIVE_BASE}/about", headers=bearer(creds), params={"fields": "user,storageQuota"}, timeout=10
            )
...
def search_google_workspace(query: str, org_id: str, source: str = "all", limit: int = 5) -> list[dict]:
    creds = _creds(org_id)
    if not creds:
        return [{"error": "Google Workspace non connecté"}]
    ...
    if source in ("all", "email") and len(results) < limit:
        try:
            r = httpx.get(
                f"{_GMAIL_BASE}/messages", headers=bearer(creds), params={"q": query, "maxResults": limit}, timeout=10
            )
            r.raise_for_status()
