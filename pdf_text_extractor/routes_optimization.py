...
def get_recommendations(user: CurrentUser = Depends(require_min_role("user"))):
    ...
    def _nav(action: str) -> str:
        a = action.lower()
        if any(k in a for k in ("licen",)):
            return "licenses"
        if any(k in a for k in ("doublon", "consolid", "outil")):
            return "duplicates"
        if any(k in a for k in ("contrat", "renégo")):
            return "contracts"
        if any(k in a for k in ("automat", "process")):
            return "processes"
        return "dashboard"
    ...
    return {
        "recommendations": sorted(recs, key=lambda x: -x["savings"]),
...
async def ai_analyze(
    ...
    if effective_org == "entreprise":
        try:
            ...
            pass
    return await _ai_cost_analysis(
        user.organization_id, question, language, effective_org, dept_type=dept_type, dept_id=dept_id
    )
                ...
                "tools": [
                    {
                        ...
                        "name": a["name"],
                        "vendor": a.get("vendor"),
                        "monthly_cost": float(a.get("monthly_cost") or 0),
                    }
                    for a in cat_apps
                ],
...
def _top_opps(lics, dups, contracts, procs) -> list[dict]:
    ...
    for l in lics[:3]:
        ...
        opps.append(
            {
                "type": "license",
                "title": f"Surplus licences : {l['product_name']} ({surplus} unités{dept})",
                "savings": l["annual_savings_potential"],
    ...
    for d in dups[:2]:
        opps.append(
            {
                "type": "duplicate",
                "title": f"Outils en doublon ({d['tool_count']}) : {d['category']}",
                "savings": d["annual_savings_potential"],
    ...
    for c in contracts[:2]:
        ...
        opps.append(
            {
                "type": "contract",
                "title": f"Contrat à renégocier : {c['vendor']} ({c['days_to_renewal']}j{dept})",
                "savings": c.get("potential_savings", 0),
    ...
    for p in procs[:2]:
        opps.append(
            {
                "type": "process",
                "title": f"Automatisation : {p['name']}",
                "savings": p["annual_savings_potential"],
...
SECTOR_OPPORTUNITIES: dict[str, list[dict]] = {
    "hopital": [
        {
            "action": "Optimiser la gestion des rendez-vous médicaux avec rappels automatiques",
            "impact": "high",
            "timeline": "1-3 mois",
            "savings_pct": 0.08,
        },
        {
            "action": "Automatiser la saisie administrative des dossiers infirmiers",
            "impact": "high",
            "timeline": "3-6 mois",
            "savings_pct": 0.12,
        },
        {
            "action": "Renégocier les contrats de fournitures médicales (appels d'offres)",
            "impact": "medium",
            "timeline": "3-6 mois",
            "savings_pct": 0.15,
        },
        {
            "action": "Consolider les systèmes informatiques cliniques (DPI unique)",
            "impact": "high",
            "timeline": "6-18 mois",
            "savings_pct": 0.20,
        },
        {
            "action": "Réduire les licences logicielles inutilisées (hors systèmes cliniques)",
            "impact": "medium",
            "timeline": "1-2 mois",
            "savings_pct": 0.05,
        },
    ...
    "municipalite": [
        {
            "action": "Numériser les services citoyens (permis, paiements, demandes en ligne)",
            "impact": "high",
            "timeline": "3-6 mois",
            "savings_pct": 0.18,
        },
        {
            "action": "Renégocier les contrats de services publics et fournisseurs",
            "impact": "high",
            "timeline": "3-6 mois",
            "savings_pct": 0.15,
        },
        {
            "action": "Optimiser la maintenance préventive des actifs (véhicules, bâtiments)",
            "impact": "medium",
            "timeline": "6-12 mois",
            "savings_pct": 0.12,
        },
        {
            "action": "Consolider les fournisseurs TI municipaux",
            "impact": "medium",
            "timeline": "3-6 mois",
            "savings_pct": 0.10,
        },
        {
            "action": "Automatiser la facturation et le recouvrement des taxes",
            "impact": "medium",
            "timeline": "2-4 mois",
            "savings_pct": 0.08,
        },
    ...
    "universite": [
        {
            "action": "Auditer et réduire les licences logicielles académiques sous-utilisées",
            "impact": "high",
            "timeline": "1-2 mois",
            "savings_pct": 0.20,
        },
        {
            "action": "Automatiser les processus d'admission et de gestion des dossiers",
            "impact": "high",
            "timeline": "3-6 mois",
            "savings_pct": 0.15,
        },
        {
            "action": "Consolider les plateformes d'enseignement en ligne (LMS unique)",
            "impact": "medium",
            "timeline": "6-12 mois",
            "savings_pct": 0.12,
        },
        {
            "action": "Renégocier les contrats de bases de données et ressources académiques",
            "impact": "medium",
            "timeline": "3-6 mois",
            "savings_pct": 0.10,
        },
        {
            "action": "Optimiser l'utilisation des espaces et salles de cours",
            "impact": "low",
            "timeline": "1-3 mois",
            "savings_pct": 0.06,
        },
    ...
    "entreprise": [
        {
            "action": "Auditer et réduire les licences SaaS sous-utilisées",
            "impact": "high",
            "timeline": "1-2 mois",
            "savings_pct": 0.15,
        },
        {
            "action": "Consolider les outils doublons par catégorie (CRM, RH, communication)",
            "impact": "high",
            "timeline": "3-6 mois",
            "savings_pct": 0.20,
        },
        {
            "action": "Renégocier proactivement les contrats fournisseurs avant renouvellement",
            "impact": "medium",
            "timeline": "1-3 mois",
            "savings_pct": 0.12,
        },
        {
            "action": "Automatiser les processus administratifs récurrents à fort volume",
            "impact": "high",
            "timeline": "3-6 mois",
            "savings_pct": 0.10,
        },
        {
            "action": "Optimiser et réduire les dépenses cloud et infrastructure TI",
            "impact": "medium",
            "timeline": "1-2 mois",
            "savings_pct": 0.08,
        },
    ...
    "entrepreneur": [
        {
            "action": "Regrouper les abonnements SaaS sur des plans annuels (économie ~20%)",
            "impact": "high",
            "timeline": "1 mois",
            "savings_pct": 0.20,
        },
        {
            "action": "Remplacer les outils payants redondants par des solutions gratuites",
            "impact": "medium",
            "timeline": "1-3 mois",
            "savings_pct": 0.30,
        },
        {
            "action": "Automatiser la facturation, les relances et le suivi des paiements",
            "impact": "high",
            "timeline": "1-2 mois",
            "savings_pct": 0.15,
        },
        {
            "action": "Consolider la gestion de projets sur un seul outil centralisé",
            "impact": "medium",
            "timeline": "1-2 mois",
            "savings_pct": 0.10,
        },
...
async def _ai_cost_analysis(
    org_id: str,
    question: str,
    language: str,
    org_type: str = "entreprise",
    *,
    ...
    try:
        ...
        _dept_domain = {
            "rh": "HR and workforce optimization",
            "hr": "HR and workforce optimization",
            "finance": "financial optimization and cost control",
            "comptabilite": "accounting and financial processes",
            "it": "IT cost optimization",
            "digital": "digital transformation optimization",
            "digitalisation": "digital transformation optimization",
            "procurement": "procurement and supply chain optimization",
            "marketing": "marketing spend optimization",
            "communication": "communications optimization",
            "sales": "sales process optimization",
            "legal": "legal operations optimization",
            "operations": "operations efficiency",
            "logistique": "logistics optimization",
            "rd": "R&D efficiency and innovation",
            "support": "customer support optimization",
            "qualite": "quality management optimization",
            "audit": "audit and compliance optimization",
            "compliance": "regulatory compliance optimization",
            "manufacturing": "manufacturing efficiency",
        ...
        analysis["total_potential_savings"] = round(sum(s.get("savings", 0) for s in analysis.get("steps", [])), 2)
...
def _build_context(
    apps: list, lics: list, budget: list, org_type: str = "entreprise", *, dept_type: str | None = None
) -> str:
    total_monthly = sum(float(a.get("monthly_cost") or 0) for a in apps)
    ...
    for a in sorted(apps, key=lambda x: -float(x.get("monthly_cost") or 0))[:10]:
        lines.append(
            f"  {a['name']} ({a.get('category','?')}): ${a.get('monthly_cost',0)}/mo · {a.get('user_count',0)} users · {a.get('status','?')}"
        )
    lines.append("\nLicenses:")
    for l in lics[:10]:
        ...
        lines.append(
            f"  {l['product_name']}: {asgn}/{qty} assigned ({pct}) · ${l.get('cost_per_unit',0)}/{l.get('billing_cycle','?')}"
        )
    return "\n".join(lines)
...
def _rule_based_analysis(org_id: str, org_type: str = "entreprise") -> dict:
    ...
    for i, l in enumerate(unused[:3], 1):
        steps.append(
            {
                "step": i,
                "action": f"Réduire licences {l['product_name']} ({l['quantity']} → {l['assigned_count']})",
                "savings": l["annual_savings_potential"],
                "impact": "medium",
                "timeline": "1-3 mois",
            }
    ...
    for d in dups[:2]:
        steps.append(
            {
                "step": len(steps) + 1,
                "action": f"Consolider outils {d['category']} ({d['tool_count']} en doublon)",
                "savings": d["annual_savings_potential"],
                "impact": "high",
                "timeline": "3-6 mois",
            }
    ...
    for p in procs[:2]:
        steps.append(
            {
                "step": len(steps) + 1,
                "action": f"Automatiser processus : {p['name']} ({p['automatable_hours_monthly']}h/mois)",
                "savings": p["annual_savings_potential"],
                "impact": "high",
                "timeline": "6-12 mois",
            }
    ...
    for c in contracts[:2]:
        steps.append(
            {
                "step": len(steps) + 1,
                "action": f"Renégocier contrat {c['vendor']} avant expiration ({c['days_to_renewal']}j)",
                "savings": c.get("potential_savings", 0),
                "impact": "medium",
                "timeline": "1-3 mois",
            }
