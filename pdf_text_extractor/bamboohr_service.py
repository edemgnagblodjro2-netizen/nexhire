...
def query_bamboohr(category: str, org_id: str,
                   department: str | None = None, limit: int = 10) -> dict:
    ...
    try:
        if category == "headcount":
            r = httpx.get(f"{base}/employees/directory",
                          auth=auth, headers=headers, timeout=12)
            r.raise_for_status()
            employees = r.json().get("employees", [])
            if department:
                employees = [e for e in employees
                             if department.lower() in (e.get("department") or "").lower()]
            return {
                "effectif_total": len(employees),
                "département_filtre": department or "tous",
                "employés": [
                    {"id": e.get("id"), "nom": e.get("displayName"),
                     "poste": e.get("jobTitle"), "département": e.get("department"),
        ...
        if category in ("time_off", "congés"):
            r = httpx.get(f"{base}/time_off/requests",
            ...
            return {
                ...
                "liste": [
                    {"employé": req.get("employee", {}).get("name"),
                     ...
                     "début": req.get("start"), "fin": req.get("end")}
        ...
        if category in ("onboarding", "new_hires"):
            ...
            r.raise_for_status()
            employees = r.json().get("employees", [])
            recent = [e for e in employees
                      if (e.get("hireDate") or "") >= "2026-01-01"]
            return {
                "nouvelles_embauches_2026": len(recent),
                "liste": [
                    {"nom": e.get("displayName"), "poste": e.get("jobTitle"),
                     "département": e.get("department"), "arrivée": e.get("hireDate")}
