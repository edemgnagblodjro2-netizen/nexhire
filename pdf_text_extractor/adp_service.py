    ...
    try:
        ...
        if category == "headcount":
            r = httpx.get(
                ...
                headers=headers,
                params={
                    "$select": "associateOID,person/legalName,workerDates/originalHireDate,workAssignments/assignmentStatus"
                },
                timeout=12,
            )
            r.raise_for_status()
        ...
        if category == "payroll":
            r = httpx.get(f"{_API_BASE}/payroll/v1/payroll-output", headers=headers, params={"$top": 1}, timeout=12)
            r.raise_for_status()
            data = r.json()
            return {
                "dernière_paie": data,
        ...
        if category == "time_off":
            r = httpx.get(
                f"{_API_BASE}/time/v2/workers/time-off-requests", headers=headers, params={"$top": 20}, timeout=12
            )
            r.raise_for_status()
            requests = r.json().get("timeOffRequests", [])
            pending = [
                x for x in requests if x.get("requestStatus", {}).get("statusCode", {}).get("codeValue") == "Pending"
            ]
            return {
