...
- **TODO DNS Cloudflare — sous-domaines apps** : remplacer les enregistrements A+TXT (`erp`, `portal`, `civicai`) par des CNAME orange vers `quebec-aid-finder.replit.app`. La racine `attentezero.ca` reste en gris (A record → 34.111.179.208). Avec CNAME orange, Cloudflare gère le SSL automatiquement via Universal SSL *.attentezero.ca.
...
- Version actuelle : **v1.1.21 build115** — build EAS `c20d96c6` en cours (21 mai 2026). Fix Apple 2.1(b) : card Premium masquée sur iOS, badges account dynamiques. Réponse envoyée au Resolution Center Apple.
...
- `pnpm --filter @workspace/scripts run regen-mobile-bundle`: Régénère `data/services.ts` depuis `services-data.json` (à relancer après tout import).
...
Chaque produit CivicAI, et chaque client enterprise, dispose de sa propre session de base de données isolée.

**Produits citoyens gratuits (AttenteZéro)** : 1 seule DB partagée par produit, 1 DBA CivicAI.

**Produits B2B/SaaS (ConstructPro)** : 1 DB distincte par client enterprise, 1 DBA dédié par client. Isolation totale entre clients.

**Phase 1 — Replit (schemas PostgreSQL)** : isolation via `CREATE SCHEMA constructpro_clientA` + utilisateur dédié par client.

**Phase 2 — AWS RDS PostgreSQL `ca-central-1`** (Montréal, conformité Loi 25) — déclencher au premier contrat B2G signé, ou 10 000 utilisateurs AttenteZéro, ou premier client ConstructPro payant. Coût estimé : ~60–150 $/mois/client selon taille instance.
...
Pour tout **nouvel artifact**, le `localPort` dans `artifact.toml` **doit** être dans la plage pré-provisionnée :
...
Un port hors de cette liste donne un `DIDNT_OPEN_A_PORT` persistant — le namespace réseau n'existe pas.

## Gotchas
- **`.npmrc` for `pnpm install`**: Ensure `.npmrc` with `node-linker=hoisted` and `shamefully-hoist=true` is present in the root to avoid `expo-router` resolution issues.
...
- **Captcha MIN_AGE**: 600ms (was 2000ms — bloquait les humains rapides). À ne PAS remonter sans repenser l'UX.
- **Wrong-number reports**: Tagués `[NUMERO ERRONE - service:<id>]` dans le bug-report message → filtrable côté admin.
- **Bulk import limit** : un payload bulk de >1500 fiches retourne une réponse vide (timeout). Splitter en chunks de ~900-1000 fiches.
- **Rate-limit admin** : POST parallèle vers `/api/admin/services` limité à 60 req/IP/15min. Utiliser `/api/admin/services/bulk` pour les imports.

## Historique
Toutes les releases → voir `CHANGELOG.md`.
