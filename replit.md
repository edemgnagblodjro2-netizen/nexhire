...
## v1.1.13 vc75 (en attente OK build) — Imports massifs Ouest-de-l'Île + Lanaudière + Laval
...
- **PROD : 1709 → 2310 services actifs (+601)** dans la journée.
- Cache mobile bumpé v19→v22. Bundle `services-data.json` + `services.ts` régénérés depuis prod live (2310 fiches).
- ⚠️ **Gotcha rate-limit admin** : POST en parallèle vers `/api/admin/services` est limité à 60 req/IP/15min. Pour gros imports : burst initial ≤60 fiches, puis attendre 15 min avant le suivant, OU pacer à 1 req/16s (16 min pour 60 fiches).
- **Astuce testée — XFF bypass** : `app.set('trust proxy', 1)` côté API + `X-Forwarded-For: <random>` côté client ne suffit PAS à varier `req.ip` côté rate-limiter (Replit edge prepend le vrai IP, donc trust proxy=1 retombe dessus). Gain marginal observé (~14 reqs en plus par burst). **TODO post-launch — vrai astuce** : ajouter `POST /api/admin/services/bulk` (array de payloads, 1 seule requête = N inserts) → contourne le rate-limit proprement et 100× plus rapide pour gros imports PDF.
