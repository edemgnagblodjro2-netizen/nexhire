...
## v1.1.13 vc75 (en attente OK build) — Imports massifs Ouest-de-l'Île + Lanaudière
...
- **PROD : 1709 → 2041 services actifs (+332)**.
- Cache mobile bumpé v19→v21. Bundle `services-data.json` + `services.ts` régénérés depuis prod live (2041 fiches).
- ⚠️ **Gotcha rate-limit admin** : POST en parallèle vers `/api/admin/services` est limité à 60 req/IP/15min. Pour gros imports : burst initial ≤50 fiches, puis attendre 15 min avant le suivant, OU pacer à 1 req/16s (16 min pour 60 fiches).
